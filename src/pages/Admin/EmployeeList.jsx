...
const EmployeeList = () => {
  ...
  const handleRegister = async () => {
    ...
    setSaving(true);
    try {
      const res = await apiCall("/users/register/employee", {
        method: "POST",
        body: JSON.stringify(formData),
      });
      const data = await res.json();
      if (data.status === "success") {
        setSuccess(`Employee ${data.data.employee_id} registered successfully`);
    ...
    <Layout>
      ...
      {showForm && (
        <div className="bg-white p-5 rounded-xl shadow mb-6">
          <h3 style={{ fontWeight: "600", marginBottom: "12px" }}>Register New Employee</h3>
        ...
        <table style={{ width: "100%", textAlign: "left", fontSize: "14px", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {employees.filter(e => e.role === "employee").length === 0 ? (
              <tr><td colSpan="8" style={{ padding: "20px", color: "#94a3b8", textAlign: "center" }}>No employees registered yet</td></tr>
            ) : (
                ...
                <tr key={emp.id} style={{ borderTop: "1px solid #e2e8f0" }}>
                  ...
                  <td style={{ padding: "12px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      <img src={emp.photo_url ? `http://127.0.0.1:5000/${emp.photo_url}` : `https://ui-avatars.com/api/?name=${emp.first_name}+${emp.last_name}&size=32&background=3b82f6&color=fff`}
                  ...
                  {/* <td style={{ padding: "12px" }}>
                    {emp.is_active && (
                      <button onClick={() => handleDeactivate(emp.id, emp.employee_id)} style={{ background: "none", border: "none", cursor: "pointer" }}>
